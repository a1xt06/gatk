...
public class PlaygroundExtract {
    ...
    public static List<Tuple2<List<AlignedAssembly.AlignmentInterval>, Double>> scoreAllConfigurations(final AlignedContig alignedContig,
        ...
        if (alignedContig.alignmentIntervals.size() > 10) {
            alignmentIntervals = alignedContig.alignmentIntervals.stream()
                    .filter(alignmentInterval -> (!canonicalChromosomes.contains(alignmentInterval.referenceInterval.getContig())
                            && alignmentInterval.alignmentScore > maxCanonicalChrAlignerScore)
        ...
        final int newMaxCanonicalChrAlignerScore = alignmentIntervals.stream()
                .filter(alignmentInterval -> canonicalChromosomes.contains(alignmentInterval.referenceInterval.getContig()))
                .mapToInt(ai -> ai.alignmentScore).max().orElse(0); // possible that no mapping to canonical chromosomes

        final Comparator<AlignedAssembly.AlignmentInterval> alignedIntervalComparator =
                Comparator.comparingInt(x -> x.referenceInterval.getStart());
        // group 2: generate, and score configurations
        final List<List<AlignedAssembly.AlignmentInterval>> allConfigurations = Sets.powerSet(new HashSet<>(alignmentIntervals))
    ...
    public static Tuple2<List<AlignedAssembly.AlignmentInterval>, Double> pickAnyBestConfiguration(final AlignedContig alignedContig, final Set<String> canonicalChromosomes) {
        final int maxCanonicalChrAlignerScore = alignedContig.alignmentIntervals.stream()
                .filter(alignmentInterval -> canonicalChromosomes.contains(alignmentInterval.referenceInterval.getContig()))
                .mapToInt(ai -> ai.alignmentScore).max().orElse(0); // possible that no mapping to canonical chromosomes


        final List<Tuple2<List<AlignedAssembly.AlignmentInterval>, Double>> scoredConfiguration = scoreAllConfigurations(alignedContig, canonicalChromosomes, maxCanonicalChrAlignerScore);
    ...
    static List<List<AlignedAssembly.AlignmentInterval>> pickBestConfigurations(final AlignedContig alignedContig,
                                                                                final Set<String> canonicalChromosomes) {
    ...
    public static double computeScoreOfConfiguration(final List<AlignedAssembly.AlignmentInterval> configuration,
                                              final Set<String> canonicalChromosomes,
                                              final int maxCanonicalChrAlignerScore) {
        ...
        double tigExplainQual =
                configuration.stream()
                        .mapToDouble(alignmentInterval -> {
                            ...
                            final double weight;
                            if (canonicalChromosomes.contains(alignmentInterval.referenceInterval.getContig())) {
                                weight = alignmentInterval.mapQual/60.0;
                            } else {
                                weight = Math.max(alignmentInterval.mapQual/60.0,
                                        alignmentInterval.alignmentScore > maxCanonicalChrAlignerScore ? 1 : 0);
                            }
                            return weight * len;
                        })
                        .sum();
